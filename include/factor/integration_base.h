...
class IntegrationBase
{
  public:
    ...
    {
        noise = Eigen::Matrix<double, 18, 18>::Zero(); //噪声矩阵；然后对噪声设置为单元矩阵；
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        //delta_q是每次都要更新，因此在累加的过程中不需要重复积分；只需要对新加入的量乘以delta_q即可；
        //下行计算相对于相机帧Ck的加速度；
        ...
        if(update_jacobian)
        {
            ...
            //step_V = V;
            jacobian = F * jacobian;;
            //误差传递方程，将过去的imu数据的不确定度与当下celiangde imu shuju fusion
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        Vector3d result_delta_p;          //位移差；
        Quaterniond result_delta_q;       //旋转差；

        Vector3d result_delta_v;         //速度差；
        Vector3d result_linearized_ba;   //线性加速度计偏置；
        Vector3d result_linearized_bg;   //线性陀螺仪偏置；
        ...
        sum_dt += dt;   //用来进行相机帧间imu数据计算；
        acc_0 = acc_1;  //类似加速度边缘化；
        gyr_0 = gyr_1;  //类似陀螺仪边缘化；
    ...
    //every IntegrationBase keeps all imu_data between corresponding img_buf;
