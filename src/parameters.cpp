...
void readParameters(string config_file)
{
    ...
    fsSettings["imu_t3opic"] >> IMU_TOPIC; //Imu数据所在文件夹；

    FOCAL_LENGTH = 721.5;        //读取焦距；

    //设定解算时间，此处参数与数据集无直接关系，但可根据数据集中场景的不同进行参数调节；以平衡实时与精度；
    SOLVER_TIME = fsSettings["max_solver_time"];
    NUM_ITERATIONS = fsSettings["max_num_iterations"];
    MIN_PARALLAX = fsSettings["keyframe_parallax"]; //该参数单位为像素；
    ...
    //此处保存了imu的相关参数，需要根据不同数据集采用IMU的不同来进行调节；
    ...
    //MAX_Object_CNT = fsSettings["max_obejct_cnt"];
